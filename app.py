...
# 💡 현재가 변수를 모든 페이지에서 사용할 수 있도록 위로 올림
...
# 💡 사이드바 메뉴 3개(AI, 파라미터 랩, 매매일지) 추가 완료
...
# ==========================================
    ...
    def _pf_editor(height=355):
        ...
        df_edited = st.data_editor(pd.DataFrame(edata), disabled=["Asset", "Current Price($)", "Return(%)"], hide_index=True, use_container_width=True, height=height, column_config={"Shares": st.column_config.NumberColumn("Shares", format="%.4f"), "Avg Price($)": st.column_config.NumberColumn("Avg($)", format="%.2f"), "Current Price($)": st.column_config.NumberColumn("Current($)", format="%.2f"), "Return(%)": st.column_config.NumberColumn("Ret(%)", format="%+.2f%%")})
    ...
    def _kv(l, v, c, s=""): return f'<div style="display:flex;flex-direction:column;padding:0 20px;border-right:1px solid rgba(0,0,0,0.08);min-width:115px;"><span style="font-size:0.52em;color:{tc_label};text-transform:uppercase;">{l}</span><span style="font-size:1.1em;font-weight:700;color:{c};line-height:1.2;">{v}</span><span style="font-size:0.6em;color:{tc_muted};">{s}</span></div>'
    
    _pnl_icon = '▲' if pnl_p >= 0 else '▼'
    st.markdown(apply_theme(f'<div style="background:#FAFAF7;border:1px solid rgba(0,0,0,0.11);border-left:4px solid {r_acc};padding:13px 0;margin-bottom:14px;display:flex;align-items:center;overflow-x:auto;"><div style="padding:0 20px 0 16px;border-right:1px solid rgba(0,0,0,0.08);"><div style="font-size:0.52em;color:{tc_label};text-transform:uppercase;">AMLS V4.5</div><div style="font-family:Plus Jakarta Sans;font-size:1.1em;font-weight:800;color:{tc_heading};line-height:1;">Portfolio</div></div>{_kv("Total NAV", f"${tval_u:,.2f}", tc_heading, f"₩{tval_k:,.0f}")}{_kv("USD/KRW", f"₩{fx:,.0f}", tc_muted, "환율")}{_kv("P&L", f"{pnl_p:+.2f}%", "#059669" if pnl_p>=0 else "#DC2626", f"{_pnl_icon} ${abs(pnl_u):,.0f}")}{_kv("Regime", f"R{curr_regime}", r_acc)}<div style="margin-left:auto;padding:0 16px;"><span class="live-pulse" style="font-size:0.58em;color:{main_color};">{rt_label}</span></div></div>'), unsafe_allow_html=True)
    ...
    if display_mode == "PC":
        ...
        st.markdown(_goal_tracker_html(tval_k), unsafe_allow_html=True)
        ...
        st.markdown(apply_theme(f'<div style="margin-bottom:8px;"><span style="font-weight:600;text-transform:uppercase;">Rebalancing</span></div>'), unsafe_allow_html=True); _rebalancing_matrix(True)
