...
# ==========================================
    ...
    st.markdown(apply_theme(f'<div style="background:#FAFAF7;border:1px solid rgba(0,0,0,0.11);border-left:4px solid {r_acc};padding:13px 0;margin-bottom:14px;display:flex;align-items:center;overflow-x:auto;"><div style="padding:0 20px 0 16px;border-right:1px solid rgba(0,0,0,0.08);flex-shrink:0;"><div style="font-family:DM Mono,monospace;font-size:0.52em;color:{tc_label};letter-spacing:0.22em;text-transform:uppercase;margin-bottom:2px;">AMLS V4.5</div><div style="font-family:Plus Jakarta Sans,sans-serif;font-size:1.1em;font-weight:800;color:{tc_heading};letter-spacing:-0.3px;line-height:1;">Portfolio</div></div>{_kv("Total NAV", f"${total_val_usd:,.2f}", tc_heading, f"₩{total_val_krw:,.0f}")}{_kv("USD / KRW", f"₩{cur_fx:,.0f}", tc_muted, "환율")}{_kv("P & L", f"{pnl_pct:+.2f}%", "#059669" if pnl_pct >= 0 else "#DC2626", f"{_pnl_icon} ${abs(pnl_usd):,.0f}")}{_kv("Regime", f"R{curr_regime}  {regime_info[curr_regime][1]}", r_acc)}{_kv("투자원금", f"${invested_cost:,.0f}", tc_muted, "취득원가")}<div style="margin-left:auto;padding:0 16px;flex-shrink:0;"><span class="live-pulse" style="font-family:DM Mono,monospace;font-size:0.58em;color:{main_color};letter-spacing:0.06em;">{rt_label}</span></div></div>'), unsafe_allow_html=True)    def _lp_build():
