...
# --- 🌐 신규 리마스터: 글로벌 마켓 대시보드 (블룸버그 터미널 스타일) ---
def page_market_dashboard():
    st.title("🌐 글로벌 매크로 & 마켓 터미널")
    
    # 1. Ticker Tape (최상단 고정)
    components.html("""
    ...
    with col_left:
        with st.container(border=True):
            ...
            if not indices_df.empty:
                ...
                fig_idx.add_trace(go.Scatter(x=indices_df.index, y=indices_df['^IXIC']/indices_df['^IXIC'].iloc[0]*100, name="NASDAQ", line=dict(color='#18bc9c', width=2)))
...
# --- 포트폴리오 관리 (완벽한 로그 기반 차트 추적 로직 탑재) ---
def make_portfolio_page(acc_name):
    def page_func():
        ...
        # 🔥 완벽 복구 & 향상: 시스템 로그 기반으로 과거부터 현재까지의 계단식 자산 추이 구축 (Ledger Reconstruction)
        if total_value > 0:
            with st.container(border=True):
                ...
                try:
                    ...
                    # 현재 UI 표 상태를 오늘 날짜로 강제 주입하여 차트 끝부분 완벽 매칭
                    ...
                    # 2. 야후 파이낸스에서 거래일자 인덱스 및 가격 데이터 획득
                        ...
                        for tkr, data in applicable_state.items():
                            ...
                            if qty > 0:
                                if tkr == 'CASH': val += qty; prin += qty
                                    ...
                                    if np.isnan(price): price = avg_p # 가격 못불러왔을때 땜빵
                                    val += qty * price
                                    prin += qty * avg_p
