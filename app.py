...
if app_mode == "[1] AMLS 백테스트 (v4 vs v4.3)":
    ...
    with st.spinner('듀얼 퀀트 엔진을 가동하여 시장 데이터를 연산 중입니다...'):
        df, tickers = load_and_calculate_data(BACKTEST_START, BACKTEST_END, INITIAL_CAPITAL, MONTHLY_CONTRIBUTION)
        strategies = ['AMLS v4.3', 'AMLS v4', 'QQQ', 'QLD', 'TQQQ']
...
# =====================================================================
# [2] 내 실전 포트폴리오 관리 모드 (다중 계좌 지원)
# =====================================================================
elif app_mode == "[2] 실전 포트폴리오 관리 (다중 계좌)":
    st.title("🏦 다중 포트폴리오 실전 관제탑")
    st.markdown("가장 진보된 **AMLS v4.3**을 바탕으로 여러 계좌를 동시에 관리하고 성과를 추적합니다.")

    # --- 사이드바: 다중 계좌 관리 ---
    st.sidebar.divider()
    st.sidebar.header("🏦 내 계좌 관리")
    account_names = list(st.session_state['accounts'].keys())
    selected_acc = st.sidebar.radio("열람할 계좌 선택:", account_names)

    st.sidebar.markdown("⚙️ **새 계좌 만들기**")
    new_acc_name = st.sidebar.text_input("새 계좌 이름", placeholder="예: 연금계좌, 도깨비용")
    if st.sidebar.button("➕ 계좌 추가", use_container_width=True):
    ...
    if len(account_names) > 1:
        if st.sidebar.button(f"🗑️ '{selected_acc}' 삭제", use_container_width=True):
            del st.session_state['accounts'][selected_acc]
            save_accounts_data(st.session_state['accounts'])
            st.rerun()

    # 현재 선택된 계좌 데이터 로드 및 세션 동기화 (충돌 방지)
    curr_acc_data = st.session_state['accounts'][selected_acc]
    pf_df = pd.DataFrame(curr_acc_data["portfolio"])
    pf_df["수량 (주/달러)"] = pf_df["수량 (주/달러)"].astype(float)
    pf_df["평균 단가 ($)"] = pf_df["평균 단가 ($)"].astype(float)

    if st.session_state.get('curr_acc_name') != selected_acc:
        st.session_state['last_portfolio_df'] = pf_df.copy()
        st.session_state['curr_acc_name'] = selected_acc

    st.markdown(f"### 📋 현재 열람 중: **{selected_acc}**")
    ...
    for _, row in pf_df.iterrows():
        ...
        if shares > 0:
            if tkr == "CASH":
                ...
                prev_price = mr['prev_prices'].get(tkr, 0.0)
    ...
    with col_header1: st.markdown(f"**[ {selected_acc} 자산 기입표 ]**")
    with col_header2:
        if st.button("🔄 계좌 비우기", type="primary", use_container_width=True):
            st.session_state['accounts'][selected_acc]["portfolio"] = [{"티커 (Ticker)": t, "수량 (주/달러)": 0.0, "평균 단가 ($)": 0.0} for t in ["TQQQ", "QLD", "QQQ", "SOXL", "USD", "GLD", "CASH"]]
            st.session_state['accounts'][selected_acc]["history"] = [{"Date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Log": "🔄 시스템: 포트폴리오 전체 초기화됨"}]
            st.session_state['accounts'][selected_acc]["first_entry_date"] = None
            save_accounts_data(st.session_state['accounts'])
            st.rerun()
    ...
    with col_table:
        st.caption("💡 셀을 더블 클릭하여 수량과 평단가를 입력하세요.")
        edited_df = st.data_editor(
            pf_df, num_rows="dynamic", key=f"editor_{selected_acc}", use_container_width=True,
            column_config={
            ...
            for tkr, new_val in new_state.items():
                if tkr in old_state:
                    ...
                    if old_val['qty'] != new_val['qty']:
                        st.session_state['accounts'][selected_acc]["history"].append({"Date": now_str, "Log": f"[{tkr}] 수량 변경: {old_val['qty']} ➔ {new_val['qty']}"})
                        changes_detected = True
                    if old_val['avg_p'] != new_val['avg_p']:
                        st.session_state['accounts'][selected_acc]["history"].append({"Date": now_str, "Log": f"[{tkr}] 평단가 변경: ${old_val['avg_p']} ➔ ${new_val['avg_p']}"})
                        changes_detected = True
                else:
                    st.session_state['accounts'][selected_acc]["history"].append({"Date": now_str, "Log": f"[{tkr}] 신규 종목 추가: {new_val['qty']}주"})
                    changes_detected = True
                    if st.session_state['accounts'][selected_acc]["first_entry_date"] is None and new_val['qty'] > 0: 
                        st.session_state['accounts'][selected_acc]["first_entry_date"] = datetime.now().isoformat()
            
            for tkr in old_state.keys():
                if tkr not in new_state:
                    st.session_state['accounts'][selected_acc]["history"].append({"Date": now_str, "Log": f"[{tkr}] 종목 삭제됨"})
                    changes_detected = True

            if changes_detected:
                st.session_state['accounts'][selected_acc]["portfolio"] = edited_df.to_dict(orient="records")
    ...
    with col_seed_txt: st.markdown(f"원하시는 **총 운용 시드(목표 자산)**를 입력하면, 현재 국면 목표 비중에 맞춰 종목별 매수/매도 수량을 정확히 계산해 드립니다.")
    with col_seed_input:
        target_seed = st.number_input("🎯 총 운용 시드 입력 ($)", value=float(curr_acc_data.get("target_seed", 10000.0)), step=1000.0, format="%.2f", label_visibility="collapsed")
        if target_seed != curr_acc_data.get("target_seed"):
            st.session_state['accounts'][selected_acc]["target_seed"] = target_seed
            save_accounts_data(st.session_state['accounts'])
    ...
    if total_value > 0:
        with st.container(border=True):
            ...
            with col_date:
                user_start_date = st.date_input("계좌 매수 시작일", value=default_date)
                st.session_state['accounts'][selected_acc]["first_entry_date"] = datetime.combine(user_start_date, datetime.min.time()).isoformat()
                save_accounts_data(st.session_state['accounts'])
    ...
    with col_jnl:
        ...
        def save_journal():
            st.session_state['accounts'][selected_acc]["journal_text"] = st.session_state[f'jnl_{selected_acc}']
            save_accounts_data(st.session_state['accounts'])
        st.text_area("시장 상황 등을 기록하세요.", value=curr_acc_data.get('journal_text', ''), height=200, key=f'jnl_{selected_acc}', on_change=save_journal, label_visibility="collapsed")
