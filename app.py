...
# =====================================================================
# [4] 페이지 구성: 내 포트폴리오 관리 (시장 인텔리전스 업그레이드)
# =====================================================================
def make_portfolio_page(acc_name):
    def page_func():
        ...
        # 🔥 실시간 시장 인텔리전스 2.0 (초호화 UI)
        st.markdown("### 📡 실시간 시장 인텔리전스")
        with st.container(border=True):
            ...
            with col_l:
                ...
                # 1. VIX 게이지 + 텍스트
                vix_val = ms['vix']
                v_color = "success" if vix_val < 25 else ("warning" if vix_val < 40 else "error")
            ...
            with col_r:
                ...
                st.progress(min(max((ret_val + 20) / 40, 0.0), 1.0)) # -20% ~ +20% 기준 정규화
            ...
            if app_reg == 4:
                ...
                st.success(f"🔥 **[Regime {app_reg} 발동]** VIX가 안정적이며 이평선이 완벽한 정배열을 이룬 골디락스 상승장입니다. **공격적인 레버리지(TQQQ, SOXL) 운용**을 권장합니다.")
            else:
        ...
        with col_log1:
            ...
            st.text_area("상황 기록", value=curr_acc_data.get('journal_text', ''), key=f"j_{acc_name}", height=150, on_change=save_j, label_visibility="collapsed")
        with col_log2:
            st.markdown("**[ 시스템 로그 ]**")
...
def page_manage_accounts():
    ...
    if st.button("🚀 생성", type="primary") and new_acc:
...
pages = {
    ...
    "📊 백테스팅": [st.Page(page_amls_backtest, title="AMLS 시뮬레이터", icon="🦅"), st.Page(page_dokkaebi_backtest, title="세윤도깨비 시뮬레이터", icon="👹")],
    "🏦 내 포트폴리오": [],
    "📜 전략 가이드": [st.Page(page_strategy_specification, title="공식 명세서 (v4.3)", icon="📄")]
}

for name in st.session_state['accounts'].keys(): pages["🏦 내 포트폴리오"].append(st.Page(make_portfolio_page(name), title=name, icon="💼"))
...
with st.sidebar.expander("💾 백업/복구"):
    ...
    if up_f and st.button("⚠️ 복구 실행"): st.session_state['accounts'] = json.load(up_f); save_accounts_data(st.session_state['accounts']); st.rerun()
