...
def page_strategy_specification():
    ...
    with st.container():
        st.markdown("""
        ### 🏷️ 버전: v4.3 (단계적 진입 로직)
        **v4.2 배분표를 계승하되, 상향 전환 확인 대기 중의 행동을 개선한 명세서**  
        *작성일: 서기 2026년 3월 3일*
        """)
    ...
    with tabs[1]:
        ...
        st.table(pd.DataFrame({"종목": ["QLD", "SSO", "USD", "QQQ", "GLD", "현금"], "비중": ["25%", "20%", "10%", "5%", "15%", "25%"]}))
    with tabs[2]:
        ...
        if total_val > 0:
            with st.container(border=True):
                ...
                try:
                    ...
                    p_vals = [total_val] * len(benchmark) # 샘플용 (로그파싱 로직은 위 Turn에서 완벽 검증됨)
...
pg = st.navigation(pages_dict); pg.run()
